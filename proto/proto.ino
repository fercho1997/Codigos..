...
const char* IDcerradura = "hh22nn";     // IP del servidor
const char* conjuto = "cambulos";      // IP del servidor
...
const int pinPulsador = A0; // Pin analógico A0 para el pulsador

    unsigned long currentMillis = millis();
    String h ; //dato pa enviar
    String s ; //dato pa enviar
    int t; //dato pa enviar    
unsigned long tiempoAnterior = 0;
const unsigned long intervalo = 120000; // 2 minutos en milisegundos

////////////////////////////////////////

int getNextAvailableRow() {
  for (int i = 0; i < 8; i++) {
    if (!nameSet[i]) {
      return i;
    }
...
void handleRoot() {
 ...
 unsigned long currentMillis = millis();
  ...
  if (currentMillis - previousMillis > watchdog) {
    ...
    Serial.println(localIPChar);
    String url = "/AppCerradura/cambip.php?conj=";
    url += conjuto;
    url += "&ip=";
    url += localIPChar;
    url += "&Cerrad=";
  ...
  }  else if (server.hasArg("guardar")  && server.hasArg("Nombre")) {
    // Obtén el valor de Id como cadena
    // Intenta convertirlo a un número entero
      int selectedRow = getNextAvailableRow();
      ...
      if (selectedRow != -1) {
        ...
        if (newName.length() > 0) {
          strcpy(names[selectedRow][0], newName.c_str());
          Serial.println("Nombre guardado en la primera columna.");
          nameSet[selectedRow] = true;
          EEPROM.put((selectedRow + 1) * sizeof(bool), nameSet[selectedRow]);
          t=selectedRow + 1;
           Serial.println(t );
           Serial.println("y " );
           Serial.println( newName.c_str());
            h=newName.c_str();
          delay(2000);
        ...
        Serial.println("No hay filas disponibles. Elimine un nombre antes de agregar uno nuevo.");
      }
  ...
  server.send(200, "text/plain", response);
    ...
    else if (server.hasArg("Abrir")) { 
     Serial.println("Abrir puerta");
    
  digitalWrite(ledverde, HIGH);
       server.send(200, "text/plain", "ok" );
       
    Serial.println("Abrir puerta Remotamente");
...
void ceguridad(){
  ...
  if (currentMillis - previousMillis > watchdog) {
    ...
    String url = "/AppCerradura/ceguridad.php?cerrad=";
    url += IDcerradura;
     url += "&conj=";
    url += conjuto;
    url += "&Alerta=";
...
void enviarDatosAlServidor() {
    int i;  // Declarar i fuera del bucle
   int condicion1=true;
      if (condicion1==true) {
    for (i = 0; i < 8; i++)  {
    if (nameSet[i]) {
      WiFiClient client;  // Declarar una nueva instancia de WiFiClient

      String url = "/AppCerradura/actualizarbd.php?cerrad=";
      url += IDcerradura;
      url += "&conj=";
      url += conjuto;
      url += "&Nhuellas=";
      ...
      Serial.print("Enviando datos a la base de datos... ");
      ...
      // Wait for server response (you might want to improve this part)
      unsigned long timeout = millis();
      while (client.available() == 0) {
        if (millis() - timeout > 5000) {
          Serial.println(">>> Client Timeout !");
      ...
      // Add a delay between requests (you might want to adjust this)
      delay(1000);
    }
...
int valorAnalogico = analogRead(A0);
 Serial.println(A0);
  // Si el pulsador está presionado, muestra el mensaje en el puerto serie
   if (valorAnalogico >16) {
    Serial.println("Abrir puerta");
  ...
  // Verifica la conexión cada 3 minutos
  if (tiempoActual - tiempoAnterior >= intervalo) {
    tiempoAnterior = tiempoActual;

    if (WiFi.status() != WL_CONNECTED) {
      // Si no está conectado a una red, cambia al modo de punto de acceso
      Serial.println("No estás conectado a la red WiFi. Cambiando a modo AP...");
      WiFi.softAP(ssid, password);
    } else {
      Serial.println("Estás conectado a la red WiFi.");
      // Puedes realizar otras acciones aquí si está conectado a la red
    }
  }
...
void accion_3() {
  ...
  Serial.println("Por favor, escriba el número de ID (de 1 a 8) en el que desea guardar esta huella...");
  id = t;
...
uint8_t getFingerprintID() {
  ...
  if (compareRow >= 1 && compareRow <= 8) {
   ...
   s="Estado%20operativo:%20sin%20problemas."; 
